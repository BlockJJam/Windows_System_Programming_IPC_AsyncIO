...
class CAtlSvcPipeModule : public ATL::CAtlServiceModuleT< CAtlSvcPipeModule,IDS_PROJNAME >
{
	static UINT s_incIds;

	static VOID CALLBACK Handler_SockRead(PTP_CALLBACK_INSTANCE, PVOID, PVOID, ULONG, ULONG_PTR, PTP_IO);
	static VOID CALLBACK Handler_PipeRead(PTP_CALLBACK_INSTANCE, PVOID, PVOID, ULONG, ULONG_PTR, PTP_IO);
...
public :
	...
	DECLARE_REGISTRY_APPID_RESOURCEID(IDR_ATLSVCPIPE, "{ece55d59-775a-4e58-948a-a50a2e179e73}")
...
VOID CALLBACK CAtlSvcPipeModule::Handler_PipeRead(PTP_CALLBACK_INSTANCE pInst, PVOID pCtx, PVOID pov,
	ULONG ior, ULONG_PTR dwTrBytes, PTP_IO ptpIo)
{
	CAtlSvcPipeModule* pThis = (CAtlSvcPipeModule*)pCtx;
...
HANDLE CAtlSvcPipeModule::CreatePipeItem(PSOCKADDR_IN psai, HANDLE& hPipe)
{
	...
	try
	{
		...
		(
			szPipeName, FILE_FLAG_FIRST_PIPE_INSTANCE | PIPE_ACCESS_DUPLEX | FILE_FLAG_OVERLAPPED,
		...
		(
			hToken, NULL, szCmdLine, NULL, NULL, true, NORMAL_PRIORITY_CLASS | CREATE_NEW_CONSOLE,
			NULL, NULL, &si, &pi
		);
		if (!bIsOK)
			throw GetLastError();
