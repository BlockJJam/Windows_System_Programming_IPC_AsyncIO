...
PVOID GetSockExtAPI(SOCKET sock, GUID guidFn)
{
	...
	LONG lRet = WSAIoctl
	(
		sock, SIO_GET_EXTENSION_FUNCTION_POINTER, &guid, sizeof(guid), &pfnEx, sizeof(pfnEx), &dwBytes, NULL, NULL
	);
...
public :
	...
	DECLARE_REGISTRY_APPID_RESOURCEID(IDR_ATLSVCMMF, "{e642802e-d1af-4aae-950e-b9bf7f53d360}")
...
HANDLE CAtlSvcMmfModule::SpawnProcess()
{
	...
	try
	{
		...
		(
			hToken, NULL, szCmdLine, NULL, NULL, true, NORMAL_PRIORITY_CLASS | CREATE_NEW_CONSOLE, NULL, NULL, &si, &pi
		);
		if (!bIsOK)
			throw GetLastError();
	}
