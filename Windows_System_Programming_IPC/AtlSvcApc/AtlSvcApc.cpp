...
class CAtlSvcApcModule : public ATL::CAtlServiceModuleT< CAtlSvcApcModule, IDS_SERVICENAME >
{
...
public :
	...
	DECLARE_REGISTRY_APPID_RESOURCEID(IDR_ATLSVCAPC, "{ad13b91c-2163-4c4f-8cf3-60d113873001}")
...
CAtlSvcApcModule _AtlModule;

VOID CALLBACK CAtlSvcApcModule::Handler_APCChildProc(ULONG_PTR dwParam)
{
	_AtlModule.APCChildProc(LONG(dwParam));
}
void CAtlSvcApcModule::APCChildProc(LONG lSize)
{
...
HRESULT CAtlSvcApcModule::PreMessageLoop(int nShowCmd)
{
	m_hChProc = NULL;
...
HRESULT CAtlSvcApcModule::PostMessageLoop()
{
...
void CAtlSvcApcModule::RunMessageLoop()
{
...
#else
	wsprintf(szCmdLine + nLen, _T("\\IPC.exe %d %d"),
		GetCurrentThreadId(), Handler_APCChildProc);
	...
	try
	{
		...
		(
			GetCurrentProcess(), GetCurrentThread(), GetCurrentProcess(), &hMyThr, 0, false, DUPLICATE_SAME_ACCESS
		);
		if (!bIsOK)
			throw GetLastError();
		...
		(
			hToken, NULL, szCmdLine, NULL, NULL, false,
