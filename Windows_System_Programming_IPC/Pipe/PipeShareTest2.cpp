	...
	PVOID pCtx, PVOID pov, ULONG ior, ULONG_PTR dwTrBytes, PTP_IO ptpIo)
{
	...
	if (dwTrBytes > 0)
	{
		...
		// SetEventWhenCallbackReturns(pInst, hevResp); <- remove
	}
...
void ParentMainProc(HANDLE hpiShared)
{
	...
	while (true)
	{
		...
		/*
		if (bIsOK)
			WaitForSingleObject(hevResp, INFINITE);			<---- remove
		else
			cout << "PIPE : WriteFile failed: " << GetLastError() << endl;
		*/
	}
	...
	try
	{
		if (!bIsChild)
		{
			...
			(
				...
				PIPE_TYPE_MESSAGE | PIPE_READMODE_MESSAGE, 1, BUFF_SIZE, 0, INFINITE, NULL 
			);
			...
			(
				NULL, szRunCmdLine, NULL, NULL, false, CREATE_NEW_CONSOLE, NULL, NULL, &si, &pi
			);
			if (bIsOK)
			{
		...
		else
		{
			...
			(
				SHARE_PIPE, GENERIC_READ | FILE_WRITE_ATTRIBUTES, 0, NULL, OPEN_EXISTING, FILE_FLAG_OVERLAPPED, NULL
			);
			if (hpiShared == INVALID_HANDLE_VALUE)
				throw GetLastError();
