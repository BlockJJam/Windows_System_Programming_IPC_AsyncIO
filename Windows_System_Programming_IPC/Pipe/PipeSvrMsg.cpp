...
VOID CALLBACK Handler_PipeRead(PTP_CALLBACK_INSTANCE pInst, PVOID pCtx, PVOID pov, ULONG ior, ULONG_PTR dwTrBytes, PTP_IO ptpIo)
{
	...
	bIsOK = ReadFile(ppi->_pipe, ppi->_buff, sizeof(ppi->_buff), NULL, ppi);
	if (!bIsOK)
	{
		ior = GetLastError();
...
bool BeginForConnection(PPIPE_ENV ppe)
{
	...
	try
	{
		...
		(
			PIPE_NAME, PIPE_ACCESS_DUPLEX | FILE_FLAG_OVERLAPPED, PIPE_TYPE_MESSAGE | PIPE_READMODE_MESSAGE,
			PIPE_UNLIMITED_INSTANCES, BUFF_SIZE, BUFF_SIZE, NMPWAIT_USE_DEFAULT_WAIT, NULL
		);
