...
void _tmain()
{
	...
	try
	{
		...
		if (!bIsChild)
		{
			...
			(
				SHARE_PIPE, PIPE_ACCESS_OUTBOUND | FILE_FLAG_OVERLAPPED, PIPE_TYPE_BYTE, 1, BUFF_SIZE, 0, INFINITE, NULL
			);
		...
		else
		{
			...
			(
				SHARE_PIPE, GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_FLAG_OVERLAPPED, NULL
			);
			if (hpiShared == INVALID_HANDLE_VALUE)
				throw GetLastError();
