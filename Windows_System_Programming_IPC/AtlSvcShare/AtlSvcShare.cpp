...
public :
	...
	DECLARE_REGISTRY_APPID_RESOURCEID(IDR_ATLSVCSHARE, "{f322cce4-9ff4-41c1-a0b4-6b11faf3e49a}")
...
void SetObjectToLowIntegrity(HANDLE hObject)
{
	...
	try
	{
		...
		(
			hObject, SE_KERNEL_OBJECT, LABEL_SECURITY_INFORMATION, NULL, NULL, NULL, pSacl
		);
...
#ifdef _WIN64
		wsprintf(szCmdLine + nLen, _T("\\TestGUIApp.exe %I64d"), hEvent);
#else
		wsprintf(szCmdLine + nLen, _T("\\TestGUIApp.exe %d"), hEvent);
#endif
